      ...
      const setting =
        {
          ...
          toolbar: "forecolor backcolor | bold italic underline strikethrough | image | table | numlist bullist | fontsizeselect ",
          menubar: '',
          init_instance_callback:function(editor) {
            EDITOR = editor;
            console.log("Editor: " + editor.id + " is now initialized.");
            //创建完成给一个事件
            _this.$emit('editorInited');
            //用来修改样式
            setTimeout(() => {
              $('.mce-last').css({"border-width": '0'})
              $('.mce-edit-area').css({"border-width": "0 0 0"})
              $('.mce-panel').css({"border-width": "0"})
            }, 100)
