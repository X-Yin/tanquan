<template>
    <div class="launch-task">
      ...
      <wait-task-wrapper v-if="myLaunchTasks.length === 0 && isNoTaskShow"></wait-task-wrapper>
      <no-task-text v-if="myLaunchTasks.length === 0 && !isNoTaskShow"></no-task-text>
      <div class="bg-wrapper">
        <div class="task-wrapper">
          <div class="task-item" v-for="(task, index) in myLaunchTasks" :key="index">
...
<script type="text/ecmascript-6">
  ...
  import {reqMyLaunchTasks} from '../../api/index.js'
  ...
  export default {
    ...
    methods: {
            ...
            if (y + clientHeight >= top) {
              if (self.isLoadOnePage) {
                ...
                if (self.pagesNumber > self.p) {
                  self.p++
                  //请求下一页的数据
                  self.loadMore(self.p, self._getMyLaunchTasks)
                }
              }
            }
          }
      ...
      //异步获取列表数据
      async _getMyLaunchTasks (p) {
        let result = await reqMyLaunchTasks(p)
        console.log(result)
        if (result.success) {
          let tasks = this.myLaunchTasks.slice()
          if (result.data.list.length === 0) {
            this.isNoTaskShow = false
            this.loadingShow = false
          } else {
            tasks = tasks.concat(result.data.list)
            this.getMyLaunchTasks(tasks)
      ...
      questionContent (index) {
        let task = this.myLaunchTasks[index]
