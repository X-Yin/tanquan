<template>
    <div class="undertake-task">
      <el-row>
        <setting :settings="settings"></setting>
      </el-row>
      <wait-task-wrapper v-if="myUndertakeTasks.length === 0 && isNoTaskShow"></wait-task-wrapper>
      <no-task-text v-if="myUndertakeTasks.length === 0 && !isNoTaskShow"></no-task-text>
      <div class="bg-wrapper">
        <div class="task-wrapper">
          <div class="task-item" v-for="(task, index) in myUndertakeTasks" :key="index">
            ...
            <div class="bottom">
              <div class="examineAnswers" @click="popAnswerText(task)">我要回答</div>
...
<script type="text/ecmascript-6">
  ...
  import {reqMyUndertakeTasks} from '../../api/index.js'
  ...
  export default {
    data () {
      return {
        ...
        pagesNumber: 1,
        loadingShow: false,
        path: '/tanqiu/answer/underTake',
        isNoTaskShow: true,//暂无任务的文字的显示
        taskContentId: -1
      }
    },
    computed: {
    ...
    mounted () {
      this._getMyUndertakeTasks(this.p)
    },
    methods: {
      popAnswerText (task) {
        this.taskContentId = task.taskContentId
            ...
            if (y + clientHeight >= top) {
              if (self.isLoadOnePage) {
                ...
                if (self.pagesNumber > self.p) {
                  self.p++
                  //请求下一页的数据
                  self.loadMore(self.p, self._getMyUndertakeTasks)
                }
              }
            }
          }
      ...
      //异步获取列表数据
      async _getMyUndertakeTasks (p) {
        let result = await reqMyUndertakeTasks(p)
        console.log(result)
        if (result.success) {
          let tasks = this.myUndertakeTasks.slice()
          if (result.data.list.length === 0) {
            this.isNoTaskShow = false
            this.loadingShow = false
          } else {
            tasks = tasks.concat(result.data.list)
            this.getMyUndertakeTasks(tasks)
            this.pagesNumber = result.data.lastPage
            this.isLoadOnePage = true
          }
        } else if (result.msg === '用户还没有接过单') {
      ...
      questionContent (index) {
        let task = this.myUndertakeTasks[index]
