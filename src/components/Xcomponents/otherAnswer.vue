  ...
  export default {
    data () {
      return {
        p: 1,
        pagesNumber: 1,
        path: /\/tanqiu\/taskDetail\/\w*\/otherAnswer/,
        loadingShow: false,
        isLoadOnePage: true,
        taskContentId: '',
        isHaveAnswer: false
    ...
    methods: {
      ...
      collapseContent (index) {
        $(`.otherAnswer .answer-unfold-wrapper:eq(${index})`).animate({height: 0}, 200)
        setTimeout(() => {
          $(`.otherAnswer .answer-1:eq(${index})`).css('display','block')
          $(`.otherAnswer .answer-unfold-wrapper:eq(${index})`).removeClass('current')
        }, 200)
      },
      //展开接单申请的内容
      unfoldContent (index) {
        $(`.otherAnswer .answer-1:eq(${index})`).css('display','none')
        setTimeout(() => {
          ...
          $(`.otherAnswer .answer-unfold-wrapper:eq(${index})`).addClass('current').animate({height: height1 + height2 + 'px'}, 200)
        }, 20)
      },
      ...
      async _getOtherAnswer (p) {
        let id = getStorage('taskDetailId')
        ...
        if (result.success) {
          if (result.data.list.length === 0) {
            ...
            let otherAnswer = this.otherAnswer.concat(result.data.list)
            this.getOtherAnswer(otherAnswer)
            ...
            if (y + clientHeight >= top) {
              if (self.pagesNumber > self.p) {
                if (self.isLoadOnePage) {
                  ...
                  self.p++
                  //请求下一页的数据
                  self.loadMore(self.p, self._getOtherAnswer)
                }
              }
            }
          }
    ...
    watch: {
      ...
      otherAnswer () {
        if (this.otherAnswer !== []) {
          ...
          //将申请的html内容渲染到页面上
          setTimeout(() => {
            let length = this.otherAnswer.length
            for (let i = 0; i < length; i++) {
              $(`.answer-unfold:eq(${i})`).html(this.otherAnswer[i].answerContent)
            }
          }, 20)
          this.watchScroll()
        }
