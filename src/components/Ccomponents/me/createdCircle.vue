      ...
      <div v-for='(item,index) in circles' :key='index' :class="{'circle-v':!ifh,'circle-h':ifh}" class="circle-item">
        ...
        <div class="detail">
          <div class='detail1'>
            ...
            <div class="quit-h" @click="popConfirmText(item)">删除</div>
          </div>
          <div class='detail2'>
...
<script>
  ...
  import {reqUserCreatedCircle,delCircle} from '../../../api/index.js'
  import {getPosition, getStorage,deleteOne, setStorage} from '../../../api/util.js'
  ...
  export default{
    data(){
      return {
        ...
        ifh:true,
        circles: [],
        item: {},//要传入confirmText的item
        path: '/tanquan/me/createdCircle',
        loadingShow: false,
        isLoadOnePage: true,
        isNoCircleShow: true,
        p: 1,
    ...
    methods:{
      ...
      //确认删除
      confirm (item) {
        this._cancelCircle(item)
      },
      //处理删除圈子函数
      async _cancelCircle (item) {
        let result = {}
        console.log(item.circleId)
        result = await delCircle(item.circleId)
        console.log(result)
        if (result.success) {
          this.$message({
            type: 'success',
          ...
          this.circles = deleteOne(item, this.circles, 'circleId')
      ...
      async _getCircle (p) {
        let result = await reqUserCreatedCircle(p)
        console.log(result)
        if (result.success) {
