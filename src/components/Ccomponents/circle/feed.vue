    ...
    <div class="feed" v-for='(item,index) in feed' :key='index'>
      <div class="task" v-if="item.timelineLinkType === 3">
        <div class="wrapper">
          ...
          <div class="des-wrapper">
            <div class="title" @click="gotoTaskDetail(item)"><span>[任务]</span><span
            ...
            <div class="detail">
              ...
              <div class="detail2">
                <P>{{handleArticleContent(item)}}<span class="detail-btn" @click="gotoTaskDetail(item)">详情</span></P>
              </div>
            </div>
          </div>
        </div>
        <div class="icons-wrapper" style="padding-bottom: 15px">
          <icons :index="index" :mode="taskType"></icons>
        </div>
      </div>
      <div class="article" v-if="item.timelineLinkType === 1">
        ...
        <div class="icons-wrapper">
          <icons :index="index" :mode="articleType"></icons>
        </div>
      </div>
    </div>
...
<script>
  ...
  import {getCircleFeed} from '../../../api/index.js'
  ...
  export default{
    ...
    methods:{
      ...
      handleArticleContent (item) {
        ...
        let result = await getCircleFeed(circleId, p)
        console.log(result);
        if (result.success) {
