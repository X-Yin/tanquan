<template>
<div class="container helper-wrapper">
	<div class="tab">
    ...
    <no-task-text v-if="people.length === 0 && !isNoTextShow " text="暂无助学者"></no-task-text>
	<div class="helper" v-for="(item, index) in people" :key="index">
		...
		<div class="detail">
			...
			<div class="detail3">
				...
				<div class="school"><span><span style="color:#333">研究方向&nbsp;</span>{{item.userMajor}}</span></div>
		...
		<div class="action">
			<div class="btn1" @click="_getRoomInfo(item)" v-if="item.userId !== userId">私信</div><br>
...
<script>
  import {getProbeTeacher, getRoomInfo} from '../../../api/index.js'
  ...
  import loadingBtn from '../../Xcomponents/loadingBtn.vue'
	export default{
      ...
      async _getPeople (p) {
        let result = await getProbeTeacher(p)
        console.log(result)
        if (result.success) {
